...
export function SpoilerVeil({ label, children }: SpoilerVeilProps) {
  ...
  const { progress, pressing, handlers } = usePressAndHold(() => setRevealed(true), 600);
    ...
    <div className="relative">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <button
          type="button"
          {...handlers}
          className="font-ui relative flex items-center gap-1.5 overflow-hidden rounded-full border px-3 py-1.5 text-xs select-none"
          style={{
        ...
        >
          ...
          />
          <LockGlyph />
          <span className="relative z-10">{label ?? "Unlocks as you read"}</span>
        </button>
      </div>
    </div>
  );
...
function LockGlyph() {
  return (
    <svg width="11" height="11" viewBox="0 0 16 16" fill="none" aria-hidden="true" className="relative z-10 shrink-0">
